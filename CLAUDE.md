...
- `LLM_MODEL_RESPONSE_MAX_TOKENS`, HTTP/retry config (`AZURE_CLIENT_MAX_RETRIES`, `HTTP_CLIENT_{RESPONSE,CONNECT,READ,WRITE}_TIMEOUT_IN_SECONDS`, etc.). These same vars also configure the openai-java client via `OpenAiClientConfiguration` — `AZURE_CLIENT_MAX_RETRIES` → `maxRetries`, the four `HTTP_CLIENT_*` values → the request/connect/read/write phases of `com.openai.core.Timeout` (response → request). `AZURE_CLIENT_BASE_DELAY_IN_SECONDS`/`AZURE_CLIENT_MAX_DELAY_IN_SECONDS` apply **only** to the Azure SDK clients (`ClientConfiguration`): the OpenAI SDK's backoff curve is fixed (~0.5 s → 8 s with jitter, honouring `Retry-After`) and is not configurable, so those two are a documented no-op on that client.
