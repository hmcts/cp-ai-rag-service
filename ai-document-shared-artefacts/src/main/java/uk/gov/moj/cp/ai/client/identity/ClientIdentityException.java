 ...
 * missing, blank or not a valid UUID. Callers map this to HTTP 401 (mapping itself lands in MTDI-06).
 *
 * <p>Also thrown by the worker-side {@link ClientId#requireValid(String)} guard so both the HTTP
 * boundary and queue workers reject an invalid {@code clientId} with one shared exception type (MTDI-01).
 */
