...
class DocumentIngestionOrchestratorTest {
    ...
    void shouldProcessQueueMessageSuccessfully() throws Exception {
        ...
        doNothing().when(tableStorageService).upsertDocumentOutcome(anyString(), anyString(), anyString(), anyString());

        // when
        orchestrator.processQueueMessage(metadata);

        // then
        verify(tableStorageService).upsertDocumentOutcome("Contract-Agreement.pdf", "123e4567-e89b-12d3-a456-426614174000", "INGESTION_SUCCESS", "Document ingestion completed successfully");
    }
    ...
    void shouldProcessQueueMessageWithDifferentDocumentTypes() throws Exception {
        ...
        // then
        verify(tableStorageService).upsertDocumentOutcome("Burglary-IDPC.pdf", "53ac8b90-c4c8-472c-a5ee-fe84ed96047b", "INGESTION_SUCCESS", "Document ingestion completed successfully");
    }
    ...
    void shouldHandleQueueMessageWithEmptyMetadata() throws Exception {
        ...
        // then
        verify(tableStorageService).upsertDocumentOutcome("Simple-Document.pdf", "456e7890-f123-4567-8901-234567890123", "INGESTION_SUCCESS", "Document ingestion completed successfully");
    }
    ...
    void shouldHandleQueueMessageWithComplexMetadata() throws Exception {
        ...
        // then
        verify(tableStorageService).upsertDocumentOutcome("Legal-Contract-Agreement.pdf",
                "789e0123-f456-7890-abcd-ef1234567890",
                "INGESTION_SUCCESS",
