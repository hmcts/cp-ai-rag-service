...
2. **Client hardening reuses the existing env vars** (DD-4): `AZURE_CLIENT_MAX_RETRIES` → `maxRetries` (default kept at 3, DD-5) and `HTTP_CLIENT_{RESPONSE,CONNECT,READ,WRITE}_TIMEOUT_IN_SECONDS` → `com.openai.core.Timeout` request/connect/read/write (DD-6), applied in `OpenAiClientFactory` via a new `OpenAiClientConfiguration`. An optional `OPENAI_CLIENT_*` rename with legacy fallback is deferred to Stage 4.
