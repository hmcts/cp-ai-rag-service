...
| FR-11 | Stage 0 → follow-on | ~~Where the spike identifies a diagnostic gap, add equivalent filter/truncation logging to `OpenAiChatService`.~~ **Closed by decision (Mahesh, 2026-09-15): no code change.** Content filtering is guaranteed disabled on all model resources as a standing operational given, so filter-event logging is not required; the D3 gate is satisfied by the recorded decision in `04-content-filter-parity-findings.md` §4. | Withdrawn |
...
- AC-22: ~~Given FR-11 logging is added, when a filtered or truncated response occurs, then the emitted log line identifies the condition without including prompt text, document content or user query content.~~ **Not applicable — FR-11 withdrawn by decision (no logging change shipped).**
