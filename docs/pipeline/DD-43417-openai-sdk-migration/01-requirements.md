...
|----|-------|-------------|----------|
| FR-1 | Stage 1 | Introduce an `OpenAiClientConfiguration` (shared artefacts, alongside the existing `ClientConfiguration`) that reads the already-documented env vars and produces openai-java client options: `AZURE_CLIENT_MAX_RETRIES` → `maxRetries`, and `HTTP_CLIENT_RESPONSE/CONNECT/READ/WRITE_TIMEOUT_IN_SECONDS` → a `com.openai.core.Timeout` with request/connect/read/write phases. | Must |
...
- AC-2: Given `HTTP_CLIENT_RESPONSE/CONNECT/READ/WRITE_TIMEOUT_IN_SECONDS` are set, when the client is built, then the applied `Timeout` carries those values in the request/connect/read/write phases respectively.
...
- AC-6: Given `AZURE_CLIENT_BASE_DELAY_IN_SECONDS` / `AZURE_CLIENT_MAX_DELAY_IN_SECONDS` are set, when the OpenAI client is built, then the build succeeds and the documentation/Javadoc states these values do not apply to this client (no silent implication of configurability).
