...
- [ ] AC-022 *(amended at code review, 2026-07-21)*: Given `getColumnsToRetrieve(clientId)`, when a search executes **with a client scope supplied**, then `IndexConstants.CLIENT_ID` is included so `clientId` round-trips into `ChunkedEntry`; when no client scope is supplied the column is **omitted** — Azure AI Search rejects a `$select` naming a field the index does not define, and the live v1 index has no `clientId` field until the v2 migration runs, so an unconditional select would break all retrieval on deploy (flag off) ahead of cut-over.
