...
| FR-18 | **[Resolved 2026-07-23]** Keep the eval harness (`ai-document-system-prompt-harness-eval`) working against a client-scoped index (D6). Resolution: satisfied with **no harness changes** — the harness stays client-unscoped (`search(null, …)` produces no filter clause and no `clientId` column reference), so it works identically against v1 and migrated v2 indexes (OQ-3 closed; design §H, DD-13 revised).                                                          | Should |
...
3. **OQ-3 — eval-harness approach (D6).** ~~Simplest approach to keep `ai-document-system-prompt-harness-eval` working against a client-scoped index (inject an incumbent `clientId`? bypass the filter? dedicated eval client?).~~ **Closed 2026-07-23 (Mahesh):** the harness evaluates prompt/model quality, not isolation — it stays client-unscoped with no changes; unscoped search works identically against v1 and v2 indexes (design §H, DD-13 revised).
