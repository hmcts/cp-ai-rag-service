...
- The new **401** on missing/invalid identity (FR-2) is an enforcement behaviour behind a flag; it is not a documented success/error schema change to a request body. The existing `requestErrored` 400/500 shapes are untouched. *Resolved 2026-07-24:* the 401 **is now documented** on all five operations (reusing `requestErrored`) as of `api-cp-ai-rag` release 0.0.15 (DD-43020); the identity header remains deliberately undocumented as a request parameter (internal APIM→functions contract, invisible to the documented consumer).
