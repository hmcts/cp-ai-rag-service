...
| `EMBEDDING_SERVICE_PROVIDER` | Embedding SDK provider toggle: `openai` (openai-java on `/openai/v1`) or `azure` (Azure OpenAI SDK, rollback leg) | `openai` |
...
| `LLM_CHAT_SERVICE_PROVIDER` | Chat SDK provider toggle: `openai` (openai-java Responses API on `/openai/v1`) or `azure` (Azure OpenAI SDK, rollback leg) | `openai` |
