...
|---|---|---|---|---|
| `AnswerRetrieval` | `HttpTrigger` POST | No explicit `route` set — host defaults the route to `AnswerRetrieval` <!-- TODO: confirm default route with the Functions host; no `route` attribute present in the @HttpTrigger annotation. The published api-cp-ai-rag contract expects `/answer-user-query`. --> | `QueueOutput` → `%STORAGE_ACCOUNT_QUEUE_ANSWER_SCORING%` | Synchronous: embeds query, searches, generates LLM answer, persists eval payload to Blob, enqueues scoring message, returns answer in HTTP response |
