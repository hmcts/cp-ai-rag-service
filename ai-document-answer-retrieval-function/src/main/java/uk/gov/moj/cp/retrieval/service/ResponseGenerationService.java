...
import static uk.gov.moj.cp.ai.langfuse.LangfuseClientFactory.compilePrompt;
import static uk.gov.moj.cp.ai.langfuse.LangfuseClientFactory.getClient;
...
//                userQueryPrompt != null ? userQueryPrompt : "");
        ...
        final Prompt systemPrompt = prompts.get("cp-summarisation-system-prompt");
        final Prompt promptForUserQuery = prompts.get(userQueryPrompt);
        String systemPromptContent = compilePrompt(systemPrompt.getText().get().getPrompt(), Map.of(
                "retrieved_documents", retrievedContextsString,
                "query_prompt", promptForUserQuery.getText().isEmpty() ? userQueryPrompt : promptForUserQuery.getText().get().getPrompt()
        ));


        Span genSpan = getTracer().spanBuilder("llm-generation").startSpan();
        genSpan.setAttribute("langfuse.prompt.name", promptForUserQuery.getText().get().getName());
        genSpan.setAttribute("langfuse.prompt.version", promptForUserQuery.getText().get().getVersion());

        genSpan.setAttribute("langfuse.observation.metadata.system_prompt_name", systemPrompt.getText().get().getName());
        genSpan.setAttribute("langfuse.observation.metadata.system_prompt_version", systemPrompt.getText().get().getVersion());
        try (Scope scope = genSpan.makeCurrent()) {
